...
include "connect.php"; ?>
<? 
$handle = fopen("/tmp/hsdSongwriters.csv", "r");
while (($data = fgetcsv($handle, 99999, ",")) !== FALSE) {
	...
	if( $groupid )
		...
		if( $data[1] == "Male" || $data[1] == "All Male Duet/Group" )
		    ...
		    {
			$sql = "update members set MemberGender = 'Female' where id in ( select memberid from group_to_member where groupid = '$groupid' )";
			echo( $sql . "<br>" );
			db_query( $sql );
		    }

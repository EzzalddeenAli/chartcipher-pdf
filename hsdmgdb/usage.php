...
<option></option>
<? 
$us = db_query_array( "select distinct( userid ) from immersionusage where userid > 0 order by lower( userid )", "userid", "userid" );
foreach( $us as $u )
{
    if( isset( $userarr[$u] ) )
        $us[$u] = $userarr[$u];
...
<option></option>
<? 
$us = db_query_array( "select distinct( ipaddress ) from immersionusage order by ipaddress", "ipaddress", "ipaddress" );
...
<option></option>
<? 
$emails = db_query_array( "select distinct( p.email ), p.id from immersionusage i, proxylogins p where i.email = p.id order by p.email", "id", "email" );

...
foreach( $quarterstorun as $q )
  {
    ...
    $numsongs = db_query_first_cell( "select count(*) from songs where id in ( $songidstr ) and IntroLengthRange <> 'No Intro' " );
    $total = 0;
    foreach( $times as $t )
      {
	$numforthis = db_query_first_cell( "select count(*) from songs where id in ( $songidstr ) and IntroLengthRange = '$t' " );
	logquery( "select count(*) from songs where id in ( $songidstr ) and IntroLengthRange = '$t' " );
	$numforthis = number_format( $numforthis * 100 / ($numsongs?$numsongs:1) );
	$sheet->write( $rownum, $colnum++, $numforthis/100, $percentformat);
    if( $addnote )
      {
          $songs = db_query_array( "select id from songs where id in ( $songidstr ) and IntroLengthRange = '$t' " );

...
$proproducts = db_query_array( "select product_id from reports_amember.am_product  where title like '% pro %'", "product_id", "product_id" );
if( !count( $proproducts ) )
    $proproducts[] = -1;
$proproducts = implode( ", ", $proproducts );

$proproducts = "59, 12, 33, 34, 57, 58, 60";



if( $go )
{
    $res = db_query_rows( "select * from immersionusage where userid in ( select user_id from reports_amember.am_access where product_id in ( $proproducts ) ) $whr order by dateadded desc" );
}
$userarr = db_query_array( "select am_user.user_id, login from reports_amember.am_user, reports_amember.am_access where am_user.user_id = am_access.user_id and product_id in ( $proproducts )", "user_id", "login" );
//echo( "select am_user.user_id, login from reports_amember.am_user, reports_amember.am_access where am_user.user_id = am_access.user_id and product_id in ( $proproducts)" );
...
<h3>Pro Usage</h3>
<form method='post' action='prousage.php'>
Filter By: <br>
<table>
<tr><td>User ID:</td><td><select name='suserid'>
<option></option>
<? 
$us = db_query_array( "select distinct( userid ) from immersionusage where userid > 0 and userid in ( select user_id from reports_amember.am_access where product_id in ( $proproducts ) )  order by lower( userid )", "userid", "userid" );
foreach( $us as $u )
{
    if( isset( $userarr[$u] ) )
        $us[$u] = $userarr[$u];
...
<option></option>
<? 
$us = db_query_array( "select distinct( ipaddress ) from immersionusage where userid in ( select user_id from reports_amember.am_access where product_id  in ( $proproducts ) ) order by ipaddress", "ipaddress", "ipaddress" );

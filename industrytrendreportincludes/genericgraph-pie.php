...
if( !$thisyearsongs )
    $thisyearsongs = getSongIdsWithinQuarter( false, $threeagoq, $threeagoy, $search["dates"]["fromq"], $search["dates"]["fromy"] );
if($doingyearlysearch)
    {
    $thisyearsongs = $allsongs;
    }

//echo( "num: " . count( $thisyearsongs ) );
// foreach( $thisyearsongs as $t )
// {
// 	echo( getSongnameFromSongid( $t ) . "<br>");
// }
...
//print_r( $thisyearsongs );
$dataforrows[$barname] = getBarTrendDataForRows( $search[comparisonaspect], "", $thisyearsongs );
    if( $_GET["rachel"] && $search["comparisonaspect"] == "Artists with More Than One Hit" )
	{
	$search["comparisonaspect"] = "Featured Artists with More Than One Hit";
	$extradataforrows = getBarTrendDataForRows( $search[comparisonaspect], "", $thisyearsongs );
	print_r( $extradataforrows );exit;
	$search["comparisonaspect"] = "Artists with More Than One Hit";
	}
...
if( $extragraphnote )
    {
    echo( "<i class='graphnote'>$extragraphnote</i><br>" );	   
    }
?>
<!-- end graph -->    
...
<? if( 1 == 1 ) { ?>
				subtitles:[
<?php
$descr = getOrCreateGraphNote( "ITR (pie): " . $sectionname, $extgraphnote );

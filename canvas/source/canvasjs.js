/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v2.2 GA - https://canvasjs.com/ 
...
function addEvent(obj, eventType, fn, store, useCapture) {
	store = store || [];
	useCapture = useCapture || false;

	//	store.push([obj, eventType, fn, useCapture]);
...
function removeEvent(obj, eventType, fn, useCapture) {
	useCapture = useCapture || false;
...
var disableWM = (window && window[fSDec("mnb`uhno")] && window[fSDec("mnb`uhno")].href && window[fSDec("mnb`uhno")].href.indexOf && (window[fSDec("mnb`uhno")].href.indexOf(fSDec("b`ow`rkr/bnl")) !== -1 || window[fSDec("mnb`uhno")].href.indexOf(fSDec("gdonqhy/bnl")) !== -1 || window[fSDec("mnb`uhno")].href.indexOf(fSDec("gheemd")) !== -1));
var disableLnk = disableWM && (window[fSDec("mnb`uhno")].href.indexOf(fSDec("gheemd")) === -1);
function addCreditLink(chart) {
	...
	if (!chart._creditLink) {
		...
		chart._creditLink.setAttribute("title", "JavaScript Charts");
...
CanvasJS.Chart = (function () {
	...
	function exportCanvas(canvas, format, fileName, exportid) {
...
//	    alert( exportid );
        if( fileName == "Chart" )
        {
	    //            alert( "exportid: " + exportid );
            eval( "var tmpchart = " + exportid + "chart;" );
	    //            alert( "tmpchart = " + tmpchart );
            if( tmpchart )
                fileName = tmpchart.options.title.text;
        }
...
// 1428, 600 for chrome
        ...
        var starting = (wid / 2) - 180;
        var heighttostart = hei - 20;
        context.fillText("© 2010 - 2021 Hit Songs Deconstructed. All rights reserved.", starting, heighttostart);
...
//		var mimeType = "image/" + format;
	...
	Chart.prototype.destroy = function () {
		var allDOMEventHandlers = this.allDOMEventHandlers;
		...
		for (i = 0; i < allDOMEventHandlers.length; i++) {
			removeEvent(allDOMEventHandlers[i][0], allDOMEventHandlers[i][1], allDOMEventHandlers[i][2], allDOMEventHandlers[i][3]);
		}

		this.allDOMEventHandlers = [];
		this.removeAllEventListeners();

		while (this._canvasJSContainer && this._canvasJSContainer.hasChildNodes()) {
		...
		while (this._dropdownMenu && this._dropdownMenu.hasChildNodes()) {
			this._dropdownMenu.removeChild(this._dropdownMenu.lastChild);
		}

		this._canvasJSContainer = null;
		this.container = null;
		this.canvas = null;
		this.overlaidCanvas = null;
		this.toolTip.container = null;
		this._preRenderCanvas = null;
		this._breaksCanvas = null;
	...
	Chart.prototype._updateOptions = function () {
		...
		if (!this._dropdownMenu && this.exportEnabled && isCanvasSupported) {
			...
			var gn = this.container.id;
			// if( this._container.id != "chartContainer" )
			//			alert( gn );
			gn = gn.replace( "chartContainer", "" );
			//			alert( gn );
			if( document.getElementById( gn + "exportjpg" ) )
			    {
				addEvent(document.getElementById( gn + "exportjpg" ), "click", function () {
					exportCanvas(_this.canvas, "jpeg", _this.exportFileName, gn);
					hide(_this._dropdownMenu);
				    }, true);
			    }
	...
	Chart.prototype.renderColumn = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : Math.min((this.width * .15), this.plotArea.width / plotUnit.plotType.totalDataSeries * .9) << 0;
		...
		var barWidth = this.dataPointWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.totalDataSeries * .9) << 0;

		if (this.dataPointMaxWidth && minBarWidth > maxBarWidth)
			minBarWidth = Math.min(this.options.dataPointWidth ? this.dataPointWidth : Infinity, maxBarWidth);

		if (!this.dataPointMaxWidth && this.dataPointMinWidth && maxBarWidth < minBarWidth)
			maxBarWidth = Math.max(this.options.dataPointWidth ? this.dataPointWidth : -Infinity, minBarWidth);
	...
	Chart.prototype.renderStackedColumn = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : this.width * .15 << 0;
		...
		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.plotUnits.length * .9) << 0;
	...
	Chart.prototype.renderBar = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : Math.min((this.height * .15), this.plotArea.height / plotUnit.plotType.totalDataSeries * .9) << 0;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;

		if (!isFinite(xMinDiff))
			xMinDiff = Math.abs(plotUnit.axisX.range) * .3;

		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.height * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.totalDataSeries * .9) << 0;
	...
	Chart.prototype.renderStackedBar = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : this.height * .15 << 0;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;

		if (!isFinite(xMinDiff))
			xMinDiff = Math.abs(plotUnit.axisX.range) * .3;

		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.height * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.plotUnits.length * .9) << 0;
	...
	Chart.prototype.renderCandlestick = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : (this.width * .015);
		...
		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) * .7) << 0;
	...
	Chart.prototype.renderRangeColumn = function (plotUnit) {
		...
		var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : (this.width * .03);
		...
		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.totalDataSeries * .9) << 0;
	...
	Chart.prototype.renderError = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			var minBarWidth = this.options.dataPointMinWidth ? this.dataPointMinWidth : this.options.dataPointWidth ? this.dataPointWidth : 1;
			var maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : (isXYSwapped) ? (Math.min((this.height * .15), this.plotArea.height / (isSeriesLinkedToBar ? totalBarTypeSeries : 1) * .9) << 0) : (this.width * .3);
			if (isRangeChart)
				maxBarWidth = this.options.dataPointMaxWidth ? this.dataPointMaxWidth : this.options.dataPointWidth ? this.dataPointWidth : (isXYSwapped) ? (Math.min((this.height * .15), this.plotArea.height / (isSeriesLinkedToBar ? totalBarTypeSeries : 1) * .9) << 0) : (this.width * .03);
	...
	Chart.prototype.renderWaterfall = function (plotUnit) {
		...
		var barWidth = this.options.dataPointWidth ? this.dataPointWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) / plotUnit.plotType.totalDataSeries * .6) << 0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					var barColor = dataPoints[i].color ? dataPoints[i].color : dataPoints[i].y > 0 ? dataSeries.risingColor : dataSeries.fallingColor;
					...
					if ((typeof dataPoints[i].isIntermediateSum !== "undefined" && dataPoints[i].isIntermediateSum === true) || (typeof dataPoints[i].isCumulativeSum !== "undefined" && dataPoints[i].isCumulativeSum === true))
						currentY = dataPoints[i].y > 0 ? y1 : y2;
					else
						currentY = dataPoints[i].y > 0 ? y2 : y1;
		...
		var fcn = function ($0) {
			...
			if (key === "color") {
				if (ds.type === "waterfall")
					return dp.color ? dp.color : dp.y > 0 ? ds.risingColor : ds.fallingColor;
				if (ds.type === "error") {
	...
	Axis.prototype.convertValueToPixel = function (value) {
		...
		if (this.logarithmic)
			...
			if (this.type === "axisX")
				...
				pixel = this.conversionParameters.reference + this.conversionParameters.pixelPerUnit * (mappedValue - this.conversionParameters.minimum) + .5;
		}
		return pixel;
	}
...
//			this.container.style.pointerEvents = "none"; // this fixes no clicking
			...
			toolTipHtml += "font-family: Calibri, Arial, Georgia, serif, Rachel;";
...
//alert( "mine:" + toolTipInnerHtml );
///alert( this.contentDiv.outerHTML );
					...
					try {
						...
						if (entries[0].dataSeries.type === "waterfall")
							this.borderColor = this.contentDiv.style.borderRightColor = this.contentDiv.style.borderLeftColor = this.contentDiv.style.borderColor = this.options.borderColor ? this.options.borderColor : entries[0].dataPoint.color ? entries[0].dataPoint.color : entries[0].dataPoint.y > 0 ? entries[0].dataSeries.risingColor : entries[0].dataSeries.fallingColor;
...
//	    alert( toolTipInnerHtml );
	...
	var RenderHelper = {
		drawMarker: function (x, y, ctx, markerType, markerSize, markerColor, markerBorderColor, markerBorderThickness) {
			...
			if (ctx.setLineDash) {
				ctx.setLineDash(getLineDashArray("solid", markerBorderThickness));
			}
...
CanvasJS.Chart.version = "v2.2 GA";

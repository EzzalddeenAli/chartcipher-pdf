...
class am4AccessRequirement {
    ...
    function checkAccess(am4AccessCacheRecord $a){
        ...
        // Check period;
        if(($a->days < $this->start) || ($a->days > $this->stop)) return false;
    ...
    function __toString() {
        ...
        if($this->id){
            ...
            switch($this->type){
                case am4Access::PRODUCT : $titles = am4PluginsManager::getAMProducts(); 
                                          break;
                case am4Access::CATEGORY : $titles = am4PluginsManager::getAMCategories(); 
                                          break;
    ...
    function __construct(){
        if($this->isLoggedIn()){
            foreach((array)$this->getAccessCache() as $a){
                $this->addRecord(new am4AccessCacheRecord(
                        array(     'type'   => ($a['fn'] == 'product_id' ? self::PRODUCT : self::CATEGORY),
                                   'days'   =>  $a['days'],
                                   'id'     =>  $a['id'],
                                   'status' =>  $a['status']
                                    )                        

...
if( $newcarryfilter )
	...
	if( $newcarryfilter == "new" )
		...
		$allsongsnumber1 = array_intersect( $allsongsnumber1, $newsongs );
...
$backurl = "trend-csv-report.php?" . $_SERVER['QUERY_STRING'];
...
else
{
...

	{
	    ...
	    foreach( $allsongs as $a )
		{
		    ...
		    else if( $weektype == "weeksuntil" )
			$numweeks = db_query_first_cell( "select count(weekdateid) from song_to_weekdate, weekdates where weekdates.id = weekdateid and OrderBy <= $lastweekdateid and  songid = $a " ); 
		    else
...
if( $search["peakchart"] ) 
    {
	$rownum++; $colnum = 0;
$sheet->write( $rownum, $colnum++, "Peak Chart Position: " . $tmpvalues[$search["peakchart"]], $format_bold ); 
    }
if( $search["producer"] ) { 
	$rownum++; $colnum = 0;
...
if( $weektype == "weeksduring" )
    ...
    $sheet->write( $rownum, $colnum++, "The number of weeks in this report reflects weeks from the initial charting until the end of the selected time period." );
else
...
function getPercent( $num, $href = "", $longformat = false )
{
    ...
    if( $href && 1== 0) {
	if( $longformat )
	    return " ($perc of songs, <a '>$num</a> $son)";
	else
...
function calcmeWeek( $rows, $songstouse = array() )
{

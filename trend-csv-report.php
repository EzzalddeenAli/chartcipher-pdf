    ...
    <? if( $nomatch ) { ?>
            ...
            <a class="search-header-clear">CLEAR ALL</a>
					...
					<div class="search-body moddded">
						<form class="search-form" method='get' name="trendsearchform" id="trendsearchform" action='trend-csv-report-results.php'>
...
?>
					...
					<div class="form-row-left">
    								<label>Groupings To Use:</label>
<br><i>If downloading a line graph, please only select one.</i>
								<br><br>
<? 
foreach( getCSVPossTitles() as $tmp )
{
echo( "<input style='display: inline !important' type='checkbox' name='posstitles[]' value='$tmp' " . (in_array( $posstitles, $tmp )?"CHECKED":"") . "> " . formatCSVTitle( $tmp ) . "<br>" );
}
?>
								</div><!-- /.form-row-left -->
	...
	<div class="cf"></div>
								<div class="form-row-left">
          								...
          								<div class="form-row-right">
								<label>Staying Power: Select whether you would like only the weeks during selected time period or all time weeks.</label>
																<select name="weektype">
																	<option value="">All Time Weeks</option>
																	<option value="weeksduring">Weeks During Selected Time Period</option>
																	<option value="weeksuntil">Weeks To Date</option>
																</select>
          						</div><!-- /.form-row-right -->
